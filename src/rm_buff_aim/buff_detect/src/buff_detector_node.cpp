 ...
 */
#include "buff_detect/buff_detector_node.hpp"
...
namespace buff_detector
{
    ...
    : Node("buff_detector", options)
    {
        ...
        this->declare_parameter<int>("buff_mode", 4);
    ...
    void BuffDetectorNode::sensorMsgCallback(const auto_aim_interfaces::msg::State::SharedPtr serial_msg)
    {
        serial_mutex_.lock();
    ...
    {   
        ...
        bullet_speed = serial_msg_.v;
        shoot_delay = this->get_parameter("shoot_delay").as_int();
        if (debug_param_.using_imu)
        {
            double cy = cos(serial_msg_.yaw * 0.5);
            double sy = sin(serial_msg_.yaw * 0.5);
            double cp = cos(serial_msg_.pitch * 0.5);
            double sp = sin(serial_msg_.pitch * 0.5);
            double cr = cos(serial_msg_.roll * 0.5);
            double sr = sin(serial_msg_.roll * 0.5);
        
            src.quat.w() = cy * cp * cr + sy * sp * sr;
            src.quat.x() = cy * cp * sr - sy * sp * cr;
            src.quat.y() = sy * cp * sr + cy * sp * cr;
            src.quat.z() = sy * cp * cr - cy * sp * sr;
    ...
    std::unique_ptr<Detector> BuffDetectorNode::initDetector()
    {
        ...
        this->declare_parameter<double>("max_delta_t", 1000.0);
        ...
        string pkg_share_pth[3] = 
        {
            ...
            get_package_share_directory("buff_detect"), 
