 ...
 */
#include "buff_detect/buff_detector.hpp"
...
namespace buff_detector
{
    ...
    bool Detector::run(TaskData& src, TargetInfo& target_info)
    {
        ...
        for (auto object : objects)
        {
            ...
            auto pnp_result = coordsolver_.pnp(points_pic, rmat_imu_, target_type);
        ...
        if (trackers_.size() != 0)
        {
            ...
            {
                RCLCPP_INFO(logger_, "%.4f %.4fttt", ((src.timestamp - (*iter).now_) / 1e6), buff_param_.max_delta_t);
