 ...
 */
#include "buff_predict/buff_processor_node.hpp"
...
namespace buff_processor
{
    ...
    void BuffProcessorNode::predictorCallback(const BuffMsg& buff_msg)
    {
        ...
        aiming_msg.tracking = true;
        aiming_msg.id = 8;
        aiming_msg.armors_num = 1;
        aiming_msg.fire = 1;
        aiming_msg.position.x = gimbal_msg.meas_point_cam.x;
        aiming_msg.position.y = gimbal_msg.meas_point_cam.y;
        aiming_msg.position.z = gimbal_msg.meas_point_cam.z;
        aiming_msg.pitch = gimbal_msg.pitch;
        aiming_msg.yaw = gimbal_msg.yaw;
    ...
    std::unique_ptr<Processor> BuffProcessorNode::initBuffProcessor()
    {
        ...
        this->declare_parameter<int>("history_deque_len_cos", 100);
        this->declare_parameter<int>("history_deque_len_phase", 50);
        this->declare_parameter<int>("history_deque_len_uniform", 50);
        this->declare_parameter<double>("max_a", 8.0);
        this->declare_parameter<double>("max_rmse", 2);
